...
function Map() {
  ...
  const [savePolygonloading, setSavePolygonloading] = useState(null);
  ...
  const savePolygon = async (polygon) => {
    ...
    try {
      await axios.post('http://localhost:8000/api/savePolygon', { coordinates });
      console.log('Polygon saved to database');
    } catch (error) {
      console.error('Error saving polygon to database', error);
    }
  };
