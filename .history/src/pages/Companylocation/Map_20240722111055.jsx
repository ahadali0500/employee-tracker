...
useEffect(() => {
...
}, [state.user.company.coordinates]);
  ...
  const isPointInPolygon = (lat, lng) => {
    ...
    polygons.forEach((polygonCoords, index) => {
      const paths = polygonCoords.map(coord => new window.google.maps.LatLng(coord.lat, coord.lng));
      const polygonObj = new window.google.maps.Polygon({ paths: paths });
  
      console.log(`Checking Polygon ${index + 1}:`, paths);
  
      if (window.google.maps.geometry.poly.containsLocation(point, polygonObj)) {
        isInside = true;
        console.log(`Point is inside Polygon ${index + 1}`);
      } else {
        console.log(`Point is outside Polygon ${index + 1}`);
      }
    });
